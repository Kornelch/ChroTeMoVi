...
while success_count < itera:
    ...
    l_arm_c=[5,6,4,6,4,4,2,5,2,5,3,5,3,4,2,4,2,4,2,4,2,4,1,4,1,3,1,3,2,2,1,2,1,2,1,2,1,1,1,2,1,1,1,1,3,4] #length of chromosome's arm before decondensation, seperate by coma
    l_arm_d=[123,126,94,148,91,107,50,140,49,133,60,111,60,99,45,100,43,95,40,94,53,82,36,98,18,96,17,84,19,83,37,53,25,58,19,62,26,33,28,36,12,35,15,36,61,95] #length of chromosome's arm after decondensation, separeta by coma
    chr_pair=23 #number of chromosome pairs
    min_rad_nu=14.9 #minimal radius of nucleus
    max_rad_nu=15.1 #maksimal radius of nucleus
    min_vol_no=0.05 #minimum occupancy of nucleus by nucleolus
    max_vol_no=0.15 #maksimum occupancy of nucleus by nucleolus
    rad_bead=0.45 #radius of beads
    ...
    def new_domain(nr_k_2):
        ...
        phi = round(random.uniform(0, math.pi), 5)
        psi = round(random.uniform(0, 2 * math.pi), 5)

        xyz[0][nr_k_2] = xyz[0][nr_k_2] + 2 * rad_bead * math.cos(phi) * math.sin(psi)
        xyz[1][nr_k_2] = xyz[1][nr_k_2] + 2 * rad_bead * math.sin(phi) * math.sin(psi)
        xyz[2][nr_k_2] = xyz[2][nr_k_2] + 2 * rad_bead * math.cos(psi)
    ...
    def bead_generate_wrap(nr_k_1, ty, dl_ch): #function to restart program
        global xyz, success_count, restart
        success = False
        for i in range(1):
            xyz_copy = deepcopy(xyz)
            if bead_generate(nr_k_1, ty, dl_ch, max_iter=100000) == True:
                success = True
                break
            else:
                print 'Restarting'
...
print(restart)
print("Model generation Completed - Exiting")
